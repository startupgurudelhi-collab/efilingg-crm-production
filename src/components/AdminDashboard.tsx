    ...
    <div className="space-y-6">
      ...
      {viewTab === 'proposals' && (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-2 border-b border-slate-100 dark:border-slate-850">
            <div>
              <h3 className="font-extrabold text-sm text-slate-900 dark:text-slate-100">Dispatched Quotes History</h3>
              <p className="text-xs text-slate-500">Review quote allocations and pricing histories.</p>
            </div>
            {onTriggerProposalDraft && (
              <button
                onClick={onTriggerProposalDraft}
                className="flex items-center space-x-1.5 py-2 px-4 rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-950 font-bold hover:bg-slate-800 dark:hover:bg-slate-100 cursor-pointer transition-all text-xs"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredProposals.length === 0 ? (
              <div className="p-8 text-center text-slate-450 text-xs md:col-span-2">
                {(proposalSearch || proposalDate) ? "No matching proposals found for your search criteria." : "No corporate proposals dispatched yet. Build a proposal template anytime."}
              </div>
            ) : (
